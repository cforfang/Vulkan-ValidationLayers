...
void GpuAssisted::PreCallRecordCreateBuffer(VkDevice device, const VkBufferCreateInfo *pCreateInfo,
    ...
    if (validate_draw_indirect_count && cb_state && cb_state->modified_create_info.usage & VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT) {
        cb_state->modified_create_info.usage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
    }
}
...
void GpuAssisted::PostCallRecordCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo *pCreateInfo,
    ...
    if (((api_version >= VK_API_VERSION_1_2) && (enabled_features.core12.drawIndirectCount == VK_FALSE)) ||
        (device_extensions.vk_khr_draw_indirect_count != kEnabledByCreateinfo)) {
        device_gpu_assisted->validate_draw_indirect_count = false;
    } else {
        const char *draw_indirect_count_string = getLayerOption("khronos_validation.draw_direct_count_val");
        device_gpu_assisted->validate_draw_indirect_count =
            *draw_indirect_count_string ? !strcmp(draw_indirect_count_string, "true") : true;
    }
...
void GpuAssisted::ResetCommandBuffer(VkCommandBuffer commandBuffer) {
    ...
    for (const auto &buffer_info : gpuav_buffer_list) {
        ...
        if (buffer_info.pre_draw_resources.new_count_buffer.buffer) {
            vmaDestroyBuffer(vmaAllocator, buffer_info.pre_draw_resources.new_count_buffer.buffer,
                             buffer_info.pre_draw_resources.new_count_buffer.allocation);
        }
...
void GpuAssisted::PreCallRecordCmdDrawIndirectCountKHR(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
                                                       ...
                                                       uint32_t stride, void *cdic_state) {
    ValidationStateTracker::PreCallRecordCmdDrawIndirectCountKHR(commandBuffer, buffer, offset, countBuffer, countBufferOffset,
                                                                 maxDrawCount, stride);
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, CMD_DRAWINDIRECTCOUNT, cdic_state);
}
...
void GpuAssisted::PreCallRecordCmdDrawIndexedIndirectCountKHR(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
                                                              ...
                                                              uint32_t maxDrawCount, uint32_t stride, void *cdic_state) {
    ValidationStateTracker::PreCallRecordCmdDrawIndexedIndirectCountKHR(commandBuffer, buffer, offset, countBuffer,
                                                                        countBufferOffset, maxDrawCount, stride);
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, CMD_DRAWINDEXEDINDIRECTCOUNT, cdic_state);
}
...
void GpuAssisted::AllocatePreDrawValidationResources(GpuAssistedDeviceMemoryBlock output_block,
                                                     ...
                                                     VkPipeline *pPipeline, cmd_draw_indirect_count_api_state *cdic_state) {
    VkResult result;
    if (!pre_draw_validation_state.globals_created) {
        auto shader_module_ci = LvlInitStruct<VkShaderModuleCreateInfo>();
        ...
        // 0 - output buffer, 1 - count buffer, 2 - new count buffer
        bindings.push_back(binding);
        binding.binding = 1;
        bindings.push_back(binding);
        binding.binding = 2;
    ...
    result = vmaCreateBuffer(vmaAllocator, &buffer_info, &alloc_info, &resources.new_count_buffer.buffer,
                             &resources.new_count_buffer.allocation, nullptr);
    if (result != VK_SUCCESS) {
        ReportSetupProblem(device, "Unable to create new count buffer.  Aborting GPU-AV");
        aborted = true;
        return;
    }
    ...
    // Original count buffer
    buffer_infos[1].buffer = cdic_state->count_buffer;
    buffer_infos[1].offset = 0;
    buffer_infos[1].range = VK_WHOLE_SIZE;
    // New count buffer
    buffer_infos[2].buffer = resources.new_count_buffer.buffer;
    buffer_infos[2].offset = 0;
    buffer_infos[2].range = sizeof(uint32_t);
    ...
    DispatchUpdateDescriptorSets(device, 3, desc_writes, 0, NULL);

    cdic_state->count_buffer = resources.new_count_buffer.buffer;
}

void GpuAssisted::AllocateValidationResources(const VkCommandBuffer cmd_buffer, const VkPipelineBindPoint bind_point,
                                              CMD_TYPE cmd_type, void *cdic_state) {
    ...
    if ((cmd_type == CMD_DRAWINDIRECTCOUNT || cmd_type == CMD_DRAWINDEXEDINDIRECTCOUNT) && validate_draw_indirect_count) {
        ...
        auto indirect_count_state = (cmd_draw_indirect_count_api_state *)cdic_state;
        AllocatePreDrawValidationResources(output_block, pre_draw_resources, state, &validation_pipeline, indirect_count_state);
        if (aborted) return;
        ...
        if (indirect_count_state->count_buffer_offset > std::numeric_limits<uint32_t>::max()) {
        ...
        BUFFER_STATE *buffer_state = GetBufferState(indirect_count_state->buffer);
        uint32_t max_count;
        uint64_t bufsize = buffer_state->createInfo.size;
        uint64_t first_command_bytes = struct_size + indirect_count_state->offset;
        if (first_command_bytes > bufsize) {
            max_count = 0;
        } else {
            max_count = 1 + ((bufsize - first_command_bytes) % indirect_count_state->stride);
        }
        ...
        pushConstants[1] = static_cast<uint32_t>(indirect_count_state->count_buffer_offset);
        indirect_count_state->count_buffer_offset = 0;
        ...
        // Make sure new count buffer finished being written before DrawIndirectCount call
        // Note that because this is inside a renderpass, a self dependency is technically necessary, but at the time of this
        // writing, there is no legal way to create a self dependency between VERTEX_SHADER and DRAW_INDIRECT stages
        // VUID-VkSubpassDependency-srcSubpass-03087
        ...
        DispatchCmdPipelineBarrier(cmd_buffer, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT, 0, 1,
                                   &memory_barrier, 0, nullptr, 0, nullptr);
